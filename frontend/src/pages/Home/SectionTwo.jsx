    ...
    <section className="w-full h-auto lg:h-screen flex justify-center items-center py-5 lg:py-14">
      <div className="w-full h-auto lg:h-full flex flex-col gap-16 lg:grid lg:grid-cols-2 lg:gap-28">
        ...
        <div className="flex flex-col justify-between w-full h-full">
          <h1 className="text-secondary font-semibold text-[2rem] md:text-[3rem] xl:text-[4rem] leading-tight">
        ...
        {/* --Right Side--  */}
        <div className="h-full w-full grid grid-flow-row  md:grid-cols-2 gap-y-10 md:gap-y-24 md:gap-x-12">
            {excellenceData.map((item,index)=>(
                <div key={index} className="flex flex-col justify-between">
                    ...
                    <h2 className="text-[#0C0E12] font-semibold text-xl 2xl:text-2xl">{item.heading}</h2>
                    <p className="text-tertiary text-sm 2xl:text-lg font-normal">{item.description}</p>
                </div>
            ))}
        </div>
      </div>
