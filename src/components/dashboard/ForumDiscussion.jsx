  ...
  <div className="hidden md:block w-64 bg-white p-5 fixed right-0 h-screen overflow-y-auto">
    ...
    <div className="flex flex-wrap gap-2">
      {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
        <div
          key={i}
          className="h-7 bg-gray-200 rounded-full w-20 animate-pulse"
        ></div>
      ))}
    </div>
  </div>
...
const ForumDiscussion = () => {
      ...
      try {
        ...
        if (response.success) {
          ...
          if (Array.isArray(response.data)) {
            ...
            threadsArray = Array.isArray(response.data)
              ? response.data
              : [response.data];
          } else {
        ...
        if (retryAttempt < MAX_RETRY_ATTEMPTS) {
          console.log(
            `Retrying... Attempt ${retryAttempt + 1}/${MAX_RETRY_ATTEMPTS}`
          );
  ...
  const handleEditComment = useCallback(
    async (commentId, threadId, newContent) => {
      const currentUserId = getCurrentUserId();
      if (!currentUserId) {
  ...
  const handleAddReply = useCallback(
    async (commentId, threadId, replyContent) => {
      const currentUserId = getCurrentUserId();
      if (!currentUserId) {
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (searchTerm !== "") {
        ...
        // Reset to show all threads when search is cleared
        loadThreads();
      }
    }, 300); // Reduced debounce time for better responsiveness

    return () => clearTimeout(timeoutId);
  }, [searchTerm, handleSearch, loadThreads]);
  ...
  return (
    <div className="md:flex md:min-h-screen md:bg-white">
      ...
      <div className="md:flex-1 md:ml-64">
        {/* Header - Fixed */}
        <div className="p-5 md:flex md:justify-between md:items-center border-b border-gray-200 bg-white md:fixed md:w-[calc(100%-256px)] mt-16 md:mt-0 md:z-10 md:top-0">
          ...
          <div className="flex gap-3 mt-1 md:mt-0">
            ...
            />
            <Link href="/forum/create">
              <button className="hidden md:block w-36 md:w-auto md:h-auto h-10 md:px-4 md:py-2 bg-[#00A99D] text-white rounded-md hover:bg-[#008F84] transition-colors">
                Create Thread
              </button>
              <button className="block md:hidden  h-9 md:w-auto md:h-auto  px-4 py-2 bg-[#00A99D] text-white rounded-md hover:bg-[#008F84] transition-colors">
                Create
              </button>
            </Link>
          </div>
                  ...
                  >
                    ...
                    <div className="md:flex md:items-center md:gap-6 text-gray-500 text-sm flex  gap-2">
                      ...
                      >
                        <ArrowUpOutlined
                          className={`transform rotate-180 ${
                            actionLoading[`downvote-${post.id}`]
                              ? "animate-spin"
                              : ""
                          }`}
                        />
                      ...
                      <button
                        onClick={() => {
                          const newOpenId =
                            openCommentId === post.id ? null : post.id;
                              ...
                              <div key={comment._id} className="flex gap-3">
                                ...
                                <div className="flex-1">
                                  ...
                                  <div className="flex items-center gap-4 mt-2">
                                    <button
                                      onClick={() =>
                                        handleLikeComment(comment._id, post.id)
                                      }
                                      disabled={
                                        commentActionLoading[
                                          `like-${comment._id}`
